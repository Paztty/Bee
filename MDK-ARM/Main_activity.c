...
uint32_t led_sevice_freq = 100;
uint32_t notfication_sevice_freq = 500;
uint32_t get_adc_sevice_freq = 1;
...
{
	int32_t data;
	if(get_adc_tick % get_adc_sevice_freq == 0)
	{

    data = ADS1220_Read_WaitForData();
	//	printf("adc in hex : %x\r\n",data);
		acc = ADS1220_convertToMilliV(data);
		printf("adc in milivoltage: %f\r\n",acc);
}
	}
